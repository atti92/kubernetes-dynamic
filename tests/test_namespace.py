...
def test_namespace_init():
    pod = V1Namespace(metadata={"name": "my_namespace"})
    assert pod.metadata.name == "my_namespace"


def test_namespace_ensure(mock_client):
    mock_client.get_api.return_value.create.return_value = V1Namespace(metadata={"name": "my_namespace"})
...
def test_namespace_ensure_conflict(mock_client):
    ...
    mock_client.get_api.return_value.get.return_value = V1Namespace(metadata={"name": "my_namespace"})
    namespace = V1Namespace.ensure("my_namespace")
    assert namespace.metadata.name == "my_namespace"


def test_namespace_annotate(mock_client):
    namespace = V1Namespace(metadata={"name": "my_namespace"})
    mock_client.get_api.return_value.get.return_value = V1Namespace(
        metadata={"name": "my_namespace", "annotations": {"something": "1"}}
    )
    mock_client.get_api.return_value.patch.return_value = V1Namespace(
        metadata={"name": "my_namespace", "annotations": {"something": "1", "something_2": "2"}}
    )
