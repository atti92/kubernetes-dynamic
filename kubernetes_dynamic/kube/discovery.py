...
class Discoverer(object):
    ...
    def default_groups(self, request_resources=False):
        ...
        groups["api"] = {
            "": {
                "v1": (
                    ResourceGroup(True, resources=self.get_resources_for_api_version("api", "", "v1", True))
        ...
        groups[DISCOVERY_PREFIX] = {"": {"v1": ResourceGroup(True, resources={"List": [ResourceList(self.client)]})}}
        return groups
        ...
        if not self._cache.get("resources") or update:
            ...
            for group in groups_response:
                ...
                for version_raw in group["versions"]:
                    ...
                    resource_group = (
                        self._cache.get("resources", {}).get(DISCOVERY_PREFIX, {}).get(group["name"], {}).get(version)
                    )
    ...
    def _load_server_info(self):
        ...
        if not self._cache.get("version"):
            try:
                self._cache["version"] = {"kubernetes": self.client.request("get", "/version", serializer=just_json)}
