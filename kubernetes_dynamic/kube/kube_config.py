...
class FileOrData(object):
    ...
    def __init__(
        self, obj, file_key_name, data_key_name=None, file_base_path="", base64_file_content=True, temp_file_path=None
    ):
...
class CommandTokenSource(object):
    ...
    def token(self):
        ...
        process = subprocess.Popen(
            [self._cmd] + self._args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
        )
...
class KubeConfigLoader(object):
        ...
        def _refresh_credentials_with_cmd_path():
            ...
            if len(cmd) == 0:
                raise ConfigException(
                    "missing access token cmd " "(cmd-path is an empty string in your kubeconfig file)"
                )
    ...
    def _refresh_oidc(self, provider):
        ...
        response = client.request(
            method="GET", url="%s/.well-known/openid-configuration" % provider["config"]["idp-issuer-url"]
        )
        ...
        try:
            ...
            if "token" in status:
                ...
                if "clientKeyData" not in status:
                    logging.error("exec: missing clientKeyData field in " "plugin output")
                    return None
        ...
        if "server" in self._cluster:
            ...
            if self.host.startswith("https"):
                ...
                if "cert_file" not in self.__dict__:
                    ...
                    self.cert_file = FileOrData(
                        self._user, "client-certificate", file_base_path=base_path, temp_file_path=self._temp_file_path
                    ).as_file()
                    self.key_file = FileOrData(
                        self._user, "client-key", file_base_path=base_path, temp_file_path=self._temp_file_path
                    ).as_file()
...
class ConfigNode(object):
    ...
    def get_with_name(self, name, safe=False):
        ...
        for v in self.value:
            if "name" not in v:
                raise ConfigException(
                    "Invalid kube-config file. " "Expected all values in %s list to have 'name' key" % self.name
                )
            if v["name"] == name:
                if result is None:
                    result = v
        ...
        if safe:
            return None
        raise ConfigException(
            "Invalid kube-config file. " "Expected object with name %s in %s list" % (name, self.name)
        )
...
def _get_kube_config_loader_for_yaml_file(filename, persist_config=False, **kwargs):
    return _get_kube_config_loader(filename=filename, persist_config=persist_config, **kwargs)
...
def load_kube_config(
    config_file=None, context=None, client_configuration=None, persist_config=True, temp_file_path=None
):
    ...
    loader = _get_kube_config_loader(
        filename=config_file, active_context=context, persist_config=persist_config, temp_file_path=temp_file_path
    )
...
def load_kube_config_from_dict(
    config_dict, context=None, client_configuration=None, persist_config=True, temp_file_path=None
):
    ...
    loader = _get_kube_config_loader(
        config_dict=config_dict, active_context=context, persist_config=persist_config, temp_file_path=temp_file_path
    )
...
def new_client_from_config(config_file=None, context=None, persist_config=True):
    ...
    load_kube_config(
        config_file=config_file, context=context, client_configuration=client_config, persist_config=persist_config
    )
    return ApiClient(configuration=client_config)


def new_client_from_config_dict(config_dict=None, context=None, persist_config=True, temp_file_path=None):
    """
